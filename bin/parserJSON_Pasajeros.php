	        	...
	        	if (preg_match($pattern, $line, $matches) === 1) {
	        		switch ($i) {
		        		case '0':
		        			...
		        			break;       		
		        		case '1':
		        			$Pasanger = array(0 => $matches[2]);
		        			break; 
		        		case '2':
		        			$Pasanger[1] = $matches[2];
		        			break; 
		        		case '3':
		        			$Pasanger[2] = $matches[2];
		        			break; 		        		
		        		case '4':
		        			$Pasanger[3] = $matches[2];
		        			break;
		        		case '5':
		        			$Pasanger[4] = $matches[2];	        					        			
		        			break;       		
		        		case '6':
		        			$Pasanger[5] = $matches[2];
		        			break; 
		        		case '8':
		        			$Pasanger[6] = $matches[3];	
		        			if ($matches[1] == "{") {
		        				$cont += 1;
		        			}	        			
		        			break;
		        		case '9':
		        			$Pasanger[7] = $matches[2];
		        			break;
		        		case '10':
		        			$Pasanger[8] = $matches[2];
		        			if ($matches[4] == "}") {
		        				$cont += 1;
		        				$doPush = ($cont % 2);
		        				if ($doPush == 0)
